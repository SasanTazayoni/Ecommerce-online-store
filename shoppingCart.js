...
function renderCartItems() {
    ...
    const totalPounds = shoppingCart.reduce((sum, entry) => {
        const item = items.find(i => entry.id === i.id);
        return sum + item.pricePounds * entry.quantity;
    }, 0);

    cartTotal.innerText = formatCurrency(totalPounds / 100);
    ...
    shoppingCart.forEach(entry => {
        ...
        price.innerText = formatCurrency(item.pricePounds * entry.quantity / 100);
